...
[![Release](https://img.shields.io/github/v/release/externpro/externpro)](https://github.com/externpro/externpro/releases)
...
1. **[externpro](https://github.com/externpro/externpro) (core)** 🧰
   CMake build platform + dependency provider model + reusable workflows (`xpsync`, `xpbuild`, `xptag`, `xprelease`).
...
3. **[tutorial](https://github.com/externpro/tutorial) (adoption walkthrough)** 🎓
   Vendor externpro as a `.devcontainer` submodule, run `xpSync`, follow `xpTag → xpBuild → xpRelease`, and consume externpro-produced deps via `find_package()`.
...
1. Run the bootstrap script to set up the xpro branch, copy workflow templates, and configure externpro
1. Verify the setup works locally with `cmake --list-presets` and `cmake --preset=<platform>`
1. Run the `xpSync` workflow to sync externpro and open a PR that standardizes presets, workflows, and repo wiring
